...
def talker():
    ...
    pub = rospy.Publisher('leg_robot_trajectory_controller/command', JointTrajectory, queue_size=10)
    rospy.sleep(0.5)

    msg = JointTrajectory()
    msg.header.stamp = rospy.Time.now()
    msg.joint_names = [ "shoulder1_joint", "shoulder2_joint", "knee_joint" ]
    ...
    msg.points[1].positions = [0.0, math.pi/6.0, math.pi/6.0]
    msg.points[1].time_from_start = rospy.Time(2.0)
    msg.points[2].positions = [0.5, math.pi/6.0, math.pi/6.0]
    msg.points[2].time_from_start = rospy.Time(3.0)
    msg.points[3].positions = [-0.5, math.pi/6.0, -math.pi/6.0]
    msg.points[3].time_from_start = rospy.Time(4.0)
    msg.points[4].positions = [0.0, math.pi/6.0, math.pi/6.0]
    msg.points[4].time_from_start = rospy.Time(5.0)
    msg.points[5].positions = [0.0, 0.0, 0.0]
